...
function Index(props) {
    ...
    const submitAll = () => {
        ...
        var now = new Date();
        var x = now
        alert("Cảm ơn Bạn vì đã hoàn thành bài Test Toeic của Astronaut Toeic vào ngày: " + now);
        //.find(v=>
        props.
            history.push({
                pathname: '/result',
    ...
    const loadQues = (part) => {
        ...
        try {
            return list.map((v, k) => {
                ...
                num = finalStt
                let idAnswer = id
                return <Card className='cardquestion' key={k} name={part}
                >
...

                    {
                        v.a.map((v, i) => {
                            ...
                            return <div className="question" key={i}>
                                <h3>{stt}</h3>
                                <p className='para-body'>{v.content}</p>
                                <Radio.Group className="radioques" name={stt} onChange={(e) => onChange(e, crAnswer, part)}>
                                    <Radio   className='radiobutton'style={radioStyle} name={stt} value={v.a}>
                                        (A) {v.a}
                                    </Radio>
                                    <Radio  className='radiobutton'style={radioStyle} name={stt} value={v.b}>
                                        (B) {v.b}
                                    </Radio>
                                    <Radio className='radiobutton' style={radioStyle} name={stt} value={v.c}>
                                        (C) {v.c}
                                    </Radio>
                                    {v.d && (
                                        <Radio className='radiobutton' style={radioStyle} name={stt} value={v.d}>
                                            (D) {v.d}
                                        </Radio>
                                    )}
         ...
         <React.Fragment><CountdownTimer
            ...
            <div className='test-form'>
                <div className="test-form-body">             
                    ...
                    <h2>PART 7</h2>
                    {loadQues(7)}
                    <Button className='test-form-body-btn' onClick={() => submitAll()} >Nộp Bài</Button>
